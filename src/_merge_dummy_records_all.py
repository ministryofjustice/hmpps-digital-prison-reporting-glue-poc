...
table_list = ["offenders", "offender_booking"]
# configuration
config_dict = dict(
...
def update_config(tablename=None):
    """
    Update configuration with elements describing paths to data
    :return: None
    """
    t_tablename = config_dict["table"]
...
def start():
    ...
    for t_table in table_list:
        ...
        un_df = union_dfs(prime_df=local_df_i, df_list=[local_df_u, local_df_d])

        df_list.append(un_df)

    all_df = union_dfs(prime_df=df_list[0], df_list=df_list[1:])

    glueContext.write_dynamic_frame.from_options(
        frame=all_df,
        connection_type="s3",
        connection_options={"path": "s3://{}/".format(config_dict["write_path"])},
        format="json",
    )
