...
def get_distinct_column_values_from_df(frame, column_of_interest):

    out_list = []

    row_array = frame.select(column_of_interest).distinct().collect()
    for _row in row_array:
        out_list.append(_row[column_of_interest])
...
def start():
    ...
    table_list = get_distinct_column_values_from_df(frame=df_event_log_in, column_of_interest="table")
